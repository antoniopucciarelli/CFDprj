...
\centering
\begin{tikzpicture}[font=\small, thick, align=center, node distance=0.42cm, x=\textwidth, y=\textheight]
...
\node[draw, 
    below=of block5
    ] (block6) {variables updating for corrector \\ $\boldsymbol{u}_{n+1} = \boldsymbol{u}_{iter} = \boldsymbol{u}^*$ };
...
\node[draw, 
    fit= (blockC) (block6), 
    label=left:\rotatebox{90}{internal corrector for $\boldsymbol{u}_{n + 1}$ and $p_{n + 1}$}, 
    minimum width=9cm,
    minimum height=8cm,
    ] (blockU) {};
...
\coordinate[right=1.2cm of block6]   (c7)                       {};
\coordinate[]                        (c8) at (c7 |- blockC)     {};

% CFL check
\node[draw
    ] (CFLcheck) at (c2) {\verb|PISO| CFL test (\ref{eqn:CFL}) \\ computing $\Delta t$};
...
\draw[-latex] (block6)    to (c7) to (c8) to (blockC);
\draw[-latex] (blockC)    to (block5);

\end{tikzpicture}
    \caption{$\mathtt{PISO}$ algorithm.}
    \label{alg:PISO}
